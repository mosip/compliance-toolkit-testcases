...
- `compliance_test_definitions_sbi.json` — 200 test cases for SBI ([Secure Biometric Interface](https://docs.mosip.io/develop/biometrics/secure-biometric-interface)) — MOSIP's protocol for biometric capture devices (fingerprint, iris, face) to expose device capabilities and perform trusted capture for registration/authentication client apps; SBI 1.0 covers registration devices, SBI 2.0 covers authentication devices
